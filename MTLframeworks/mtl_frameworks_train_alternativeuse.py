...
# torch.cuda.set_device(0) # 可能移除这一行，可能会影响程序正常运行。不是必须指定为0的GPU。
...
class TaskMoE(nn.Module):
    def __init__(self, backbone_model, dim):
        ...
        self.backbone = backbone_model
        # 每个任务的专家模块
        self.expert1 = nn.Sequential(nn.Linear(dim, dim), nn.ReLU(), nn.Linear(dim, 10)).cuda()
        self.expert2 = nn.Sequential(nn.Linear(dim, dim), nn.ReLU(), nn.Linear(dim, 6)).cuda()
        self.expert3 = nn.Sequential(nn.Linear(dim, dim), nn.ReLU(), nn.Linear(dim, 2)).cuda()
        self.gate = nn.Linear(dim, 3).cuda()  # 门控网络，用于选择专家
        self.criterion_cls = nn.CrossEntropyLoss().cuda()
    ...
    def forward(self, x, ds, label):
        ...
        for i, d in enumerate(ds):
            y = label[i].unsqueeze(0).cuda()
            # 门控网络计算专家权重
            gate_weights = torch.softmax(self.gate(feature[i].unsqueeze(0)), dim=1)
            
            # 根据任务选择对应的专家模块并应用权重
            if d == 0:  # Task 1
                task_output = gate_weights[0, 0] * self.expert1(feature[i].unsqueeze(0))
            elif d == 1:  # Task 2
                task_output = gate_weights[0, 1] * self.expert2(feature[i].unsqueeze(0))
            elif d == 2:  # Task 3
                task_output = gate_weights[0, 2] * self.expert3(feature[i].unsqueeze(0))
            else:
                raise ValueError("Invalid task ID")

            # 计算预测和损失
            _, pred = torch.max(task_output.data, 1)
            loss += self.criterion_cls(task_output, y)
            predictions.append(pred)
...
def initialize_model(model_name, framework_name):
    ...
    elif framework_name == "TaskMoE":
        model = TaskMoE(backbone_model, dim).cuda()
    else:
...
class ExGAN():
    def __init__(self, model, preprocess, model_name, framework_name):
        self.model = model
        ...
        self.log_write = open(f"./results/log_{self.model_name}_{self.framework_name}_{self.date_str}_results.txt", "w")
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            if test_acc > best_accuracy:
                ...
                # 保存最佳权重
                torch.save(self.model.state_dict(), f"saved_models/{self.model_name}_{self.framework_name}_{self.date_str}_best.pth")
    ...
    def test(self, dataloader):
        ...
        results_path = os.path.join(results_dir, f"{self.model_name}_{self.framework_name}_{self.date_str}_results.csv")
...
def main():
    ...
    for model_name in model_names:
        for framework_name in frameworks:
            ...
            exgan = ExGAN(model, preprocess, model_name, framework_name)
            exgan.train()
            wandb.finish()
